...
class tweetStreamer(TwythonStreamer):
    ...
    def store_json(self):
        with open('tweet_stream_{}_{}_{}.json'.format('trump','CA', len(collectedTweets)), 'w') as f:
...
if __name__ == '__main__':
    ...
    while(len(collectedTweets)<1000):
        ...
        try:
            #Filter by location using the latitude and longitude of southwest and north east points from the state of California
            tweetCollection.statuses.filter(locations=[-116.93,32.64,-116.80,36.41])
        except:
