...
class tweetStreamer(TwythonStreamer):
    ...
    def store_json(self):
        with open('tweet_stream_{}_{}_{}.json'.format('trump','NY', len(collectedTweets)), 'w') as f:
...
if __name__ == '__main__':
    ...
    while(len(collectedTweets)<1000):
        ...
        try:
            #Filter by location using the latitude and longitude of southwest and north east points from the state of Newyork
            tweetCollection.statuses.filter(locations=[-73.953616,40.597585,-73.782355,40.911488])
        except:
